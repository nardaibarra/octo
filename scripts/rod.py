...
class Rod:
    ...
    def attach_entity(self, entity):
        ...
        # self.angular_velocity = entity.velocity[0]
    ...
    def detach_entity(self):
        if self.attached_entity:
            ...
            horizontal_velocity = self.length * self.attachment_fraction * self.angular_velocity * m.cos(self.angle)
            vertical_velocity = -self.length * self.attachment_fraction * self.angular_velocity * m.sin(self.angle)
            
            print("Horizontal: " , horizontal_velocity)
            print("Vertical: " , vertical_velocity)

            # Apply this velocity to the attached entity
            self.attached_entity.velocity[0] = horizontal_velocity + 2
            self.attached_entity.velocity[1] = vertical_velocity - 3
    ...
    def update(self):
        if self.swinging and self.attached_entity:
            ...
            angular_acceleration = -(self.gravity / self.length) * m.sin(self.angle)
